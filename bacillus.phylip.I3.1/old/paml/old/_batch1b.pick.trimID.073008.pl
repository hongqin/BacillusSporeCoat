# 073008 only for BGclusters with curated ids.tab.curated
...
foreach my $mydir ( @clusterdirs ) {
  ...
  open ( OUT, ">prot-bacillus.spec.2.faa" );
  for ( my $i=0; $i<= ($#lines -1); $i=$i+2) {
     ...
     if ( ( $myflag eq 'Y' ) && ( exists $curated_ids{$id} ) ){
  ...
  my %table = ();
  open( IN,  "<prot-bacillus.spec.2.faa" );
  open( OUT, ">prot-bacillus.10char.2.faa" );
  while( my $line = <IN> ) {
    if ( $line =~ /^>/ ) {
       chomp $line; 
       $line =~ s/_...$//o;
  ...
  open (OUT, ">_ids.bacillus.curated.csv");
