...
my $ingrepfile = 'out.4cer.Nsites/grep.lnL.txt';
open(IN, "<$ingrepfile"); my @lines = <IN>; close (IN);

open(OUT,">out.4cer.Nsites/out.coat.4cerNsites.lnL.csv");
...
foreach my $line ( @lines ) {
  ...
  my ($bg, $mydir, $h, $file, @rest) = split( /\//, $line );
  ...
  chdir "$bg/$mydir/$h";
   ...
   my $light = 'green'; 
   my @results = (); 
   #unless ( open (RES, "<results.cereus.$h.txt") ){ $light='red'};  ### Change here
   unless ( open (RES, "<$file") ){ $light='red'};  ### Change here
   if ( $light eq 'green' ) {
      @results = <RES>; 
   close (RES);
   } else {
...
#chdir $root; 
#pen( OUT, ">__omega.coat.Bha5sub4cer.011009.csv");
