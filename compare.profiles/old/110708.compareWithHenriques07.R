...
#   10    60
 ...
 annot = read.table( "/home/hqin/coat.protein07/key.data/Bacillus_subtilis_coat_proteins.092008.csv",
header=T,sep="\t");

 str(annot);
 annot$Gene = as.character( annot$Gene );
 annot$NCBI = as.character( annot$NCBI );

 annot$hamming = d[ match(annot$NCBI, names(d) ) ]

 ### which genes are in our annotation but not in Henriques07?
 annot$DriksUniq = ifelse( is.na(annot$hamming), 1, 0 );
 MyDiff = annot[is.na(annot$hamming), ]

 ### which genes have different profiles?
 annot[ annot$hamming>0 & (! is.na(annot$hamming) ),c(1:3,21:22)]
 annot[ annot$hamming>0 & (! is.na(annot$hamming) ),c(1:2,5,21)]
 ...
 Hcoat[Hcoat$HenriUniq==1,]

 ### output the new files
 write.csv(Hcoat, "Henriques07.coat.profile.110708.csv",row.names=F)
 write.csv(annot, "Bacillus_subtilis_coat_proteins.110708.csv", row.names=F)
